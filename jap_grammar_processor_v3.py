...
def has_stem_errors(text, llm, knowledge_point, memory):
    ...
    prompt = PromptTemplate(
        ...
        template = """
            ...
            the review history is attached here: 
            {history}
        """
    )
    ...
    input_data = f"""
        ...
        the question is used to test {knowledge_point}
        Check if the **question stem** (the main question part before the options) has errors, such as:
        - Grammatical mistakes
        - Unnatural sentence structures
        - Ambiguous wording
        If there is at least one issue in the stem, respond with 'True'. Otherwise, respond with 'False'.
...
def stem_options_error(text, llm, knowledge_point, memory):
    ...
    input_data = f"""
        ...
        the question is used to test {knowledge_point}
        Check if any of the following problems exists:
        ...
        If it has **stem-options problem**, respond with 'True'. Otherwise, respond with 'False'.
        Your output must be exactly 'True' or 'False', nothing else.
    """
    result = api_call_with_retry(chain, input_data)
    if result is not None:
...
def has_multiple_correct_answers(text, llm, knowledge_point, memory):
    ...
    input_data = f"""
        ...
        the question is used to test {knowledge_point}
        Check if it has **more than one correct answer**. This means that multiple options are OK for the question.
        **Important**: once an option is grammatically correct, regard it as a correct answer, even if it's not a perfect choice! In other words, never consider **tone** or **polite format**.
        ...
        If it has more than only one valid correct option, respond with 'True'. Otherwise, respond with 'False'.
        Your output must be exactly 'True' or 'False', nothing else.
    """
    result = api_call_with_retry(chain, input_data)
    if result is not None:
...
# modify module
def modify_duplicate_questions(question, llm, memory):

    prompt = PromptTemplate(
        input = ["history", "input_data"],
        template = """
**You are an experienced and strict Japanese N4/N5 examiner tasked with reviewing and ensuring that all multiple-choice test questions are in high quality and meet the following criteria.**
...
the conversation history is attached here:
{history}
        """
    )
    chain = LLMChain(llm=llm, prompt=prompt, memory=memory)
    input_data = f"""
        The questions below have **duplicate_questions problem** that need to be addressed. **Please detect questions that are really similar or even the same and change them to make them less similar**.
        For example: Two questions in the paper have over 70% of similarity.

        {question}
    """
    revised_question = api_call_with_retry(chain, input_data)

    return revised_question


def modify_stem_errors(question, llm, memory):
...
{history}
    ...
    chain = LLMChain(llm = llm, prompt = prompt, memory = memory)
    input_data = f"""
        This question has **stem problems** that need to be addressed. **Please detect and modify all possible errors**.
        For example: the stem is grammatical mistakes or has unnatural sentence structures or has ambiguous wording.
        You must ensure that the modified question won't have the following issues:
        - There are grammatical mistakes in the stem.
...
def modify_duplicate_options(question, llm, memory):
...
{history}
    ...
    chain = LLMChain(llm = llm, prompt = prompt, memory = memory)
    input_data = f"""
        This question has **duplicate_options problem** that need to be addressed. Please detect and modify all possible errors.
        For example: Two options in one questions are the same.
        You must ensure that the modified question won't have the following issues:
        - There are duplicate options in the question.
...
def modify_stem_options_error(question, llm, memory):
...
{history}
    ...
    chain = LLMChain(llm = llm, prompt = prompt, memory = memory)
    input_data = f"""
        The stem and the options of the below question don't fit well which need to be addressed. **Please detect and modify all possible errors.**
        You must ensure that the modified question won't have the following issues:
        - There is repetition between options and the stem.
        - The sentence doesn't remain natural and grammatically correct after filling in the correct answer.
...
def modify_multiple_correct_answer(question, llm, memory):
...
{history}
    ...
    chain = LLMChain(llm = llm, prompt = prompt, memory = memory)
    input_data = f"""
        This question has **multiple correct answers** that need to be addressed. **Please detect and modify all possible errors**.
        Ensure that only one option is grammatically correct for each question and all three othere options are definitely wrong.
        The criteria for correctness only include grammar. **Do not consider the tone, the politeness, the common sense.**
        **You must provide options that won't cause any confusion and ambiguity.**
...
# loop module
def question_revise_simple(rows, filename, output_dir, revised_newpaper_folder, knowledge_point, max_iterations=3, model='gpt-4o', temperature=0.6, modify_iteration=3):
    """
    ...
    for iteration in range(max_iterations):
        ...
        try:
            modify_round = 1
            while modify_round <= modify_iteration and has_duplicate_questions(revised_paper):
                print(f"find duplicate questions in iteration {iteration} - {modify_round}")

                revised_paper = modify_duplicate_questions(revised_paper, llm, memory)

                modify_round += 1
            if modify_round == 4:
                if has_duplicate_questions(revised_paper):
                    print(f"duplicate questions still exist in iteration {iteration}")
                else:
                    print(f"No duplicate questions found in iteration {iteration}")
            else:
                print(f"No duplicate questions found in iteration {iteration}")
        ...
        question_list = split_questions(revised_paper)
        new_question_paper = ""
        for question in question_list:
            ...
            try:
                modify_round = 1
                while modify_round <= modify_iteration and has_stem_errors(revised_question, llm, knowledge_point, memory):
                    print(f"find stem errors in iteration {iteration} - {modify_round}")

                    revised_question = modify_stem_errors(revised_question, llm, memory)

                    modify_round += 1
                if modify_round == 4:
                    if has_stem_errors(revised_question, llm, knowledge_point, memory):
                        print(f"stem_errors still exist in iteration {iteration}")
                    else:
                        print(f"No stem errors found in iteration {iteration}")
                else:
                    print(f"No stem errors found in iteration {iteration}")
            ...
            try:
                modify_round = 1
                while modify_round <= modify_iteration and has_duplicate_options(revised_question):
                    print(f"find duplicate options in iteration {iteration} - {modify_round}")

                    revised_question = modify_duplicate_options(revised_question, llm, memory)

                    modify_round += 1
                if modify_round == 4:
                    if has_duplicate_options(revised_question):
                        print(f"duplicate options still exist in iteration {iteration}")
                    else:
                        print(f"No duplicate options found in iteration {iteration}")
                else:
                    print(f"No duplicate options found in iteration {iteration}")
            ...
            try:
                modify_round = 1
                while modify_round <= modify_iteration and stem_options_error(revised_question, llm, knowledge_point, memory):
                    print(f"find stem options error in iteration {iteration} - {modify_round}")

                    revised_question = modify_stem_options_error(revised_question, llm, memory)

                    modify_round += 1
                if modify_round == 4:
                    if has_stem_errors(revised_question, llm, knowledge_point, memory):
                        print(f"stem_options_error still exist in iteration {iteration}")
                    else:
                        print(f"No stem_options_error found in iteration {iteration}")
                else:
                    print(f"No stem_options_error found in iteration {iteration}")
            ...
            try:
                modify_round = 1
                while modify_round <= modify_iteration and has_multiple_correct_answers(revised_question, llm, knowledge_point, memory):
                    print(f"find multiple correct answer in iteration {iteration} - {modify_round}")

                    revised_question = modify_multiple_correct_answer(revised_question, llm, memory)

                    modify_round += 1
                if modify_round == 4:
                    if has_multiple_correct_answers(revised_question, llm, knowledge_point, memory):
                        print(f"multiple correct answer error still exist in iteration {iteration}")
                    else:
                        print(f"No multiple correct answers found in iteration {iteration}")
                else:
                    print(f"No multiple correct answers found in iteration {iteration}")
            ...
            new_question_paper = new_question_paper + "\n" + revised_question

        revised_paper = new_question_paper
        intermediate_path = os.path.join(output_dir, f"{filename}_iteration_{iteration + 1}.docx")
        output_doc = Document()
        sentences = split_into_sentences(revised_paper)
        for sentence in sentences:
            output_doc.add_paragraph(sentence)
    ...
    qa_list = parse_questions(revised_paper)
...
def generate_check_store_pipeline(grammar_list, num_list, output_dir, revised_newpaper_folder, base_filepath):
    ...
    for knowledge_point, question_number in zip(grammar_list, num_list):
        ...
        for question_format in range(1, 3):
            print(f"Generating questions for format {question_format}...")
            questions_text = generate_grammar_questions(knowledge_point, question_format, 1)
...
def main():
    ...
    grammar_num = extract_numbered_content(test_grammar_original, 1, 1)[0]
    grammar_test = extract_numbered_content(test_grammar_original, 1, 1)[1]

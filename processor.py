...
class DocumentProcessor:
    ...
    def knowledge_points_match(self, rows, material_paper, filename):
        llm = ChatOpenAI(
            temperature=0.8,
            model="gpt-4o"
        )
        prompt_four = ChatPromptTemplate.from_template(
            "Now here are the knowledge points of Japanese language test :{material}, it includes vocabulary and grammar knowledge of Japanese. \
            Please use it as a reference to matching this list of questions which belongs to Japanese test exam: {error_report}. \
            For each question, keep the response concise and structured, only include the question text, options, the student's chosen answer, the correct answer, and the specific knowledge point."
            "Ensure the full analysis is provided. If the content is too lengthy to return in one response, mark the end of the first part with 'CONTINUE...' and I will prompt you for the next part."
        )
        ...
        # 分批处理错题
        batch_size = 10  # 每次处理10题
        for batch in self.split_list(rows, batch_size):
            input_four = {'material': material_paper, 'error_report': batch}
            matching_result = chain_four.run(input_four)
            all_results.append(matching_result)
    
        # 合并所有批次结果
        full_matching_result = "\n".join(all_results)

        # 保存到文件
        output_doc = Document()
        sentences = self.split_into_sentences(full_matching_result)
        for sentence in sentences:
            output_doc.add_paragraph(sentence)
    ...
    def process(self, input_paper, correct_answers_path, sample_analysis):
        ...
        for filepath in glob.glob(os.path.join(self.input_folder, "*.docx")):
            ...
            # 先跑一遍错题知识点分析
            self.knowledge_points_match(' '.join(problem_list), material, filename)
            end_time = time.time()
...
def main():
    ...
    material_folder = "C:\\Users\\刘宇\\OneDrive - CUHK-Shenzhen\\桌面\\JAP_GPT\\processed material"
    ...
    # input_folder = "C:\\Users\\刘宇\\OneDrive - CUHK-Shenzhen\\桌面\\JAP_GPT\\Test1_new\\student paper"
    input_folder = "C:\\Users\\刘宇\\OneDrive - CUHK-Shenzhen\\桌面\\JAP_GPT\\Test1_new\\student paper_test"
    output_folder = "C:\\Users\\刘宇\\OneDrive - CUHK-Shenzhen\\桌面\\JAP_GPT\\Test1_new\\New Paper"
    # output_folder = "C:\\Users\\刘宇\\OneDrive - CUHK-Shenzhen\\桌面\\JAP_GPT\\Test1_new\\New Paper_test"
    output_mistakes_folder = "C:\\Users\\刘宇\\OneDrive - CUHK-Shenzhen\\桌面\\JAP_GPT\\Test1_new\\Student Mistakes"
    output_analysis_folder = "C:\\Users\\刘宇\\OneDrive - CUHK-Shenzhen\\桌面\\JAP_GPT\\Test1_new\\Knowledge Point Analysis"
    correct_answers_path = "C:\\Users\\刘宇\\OneDrive - CUHK-Shenzhen\\桌面\\JAP_GPT\\Test1_new\\test 1 paper\\Test 1 Model Answer.docx"
    input_paper = "C:\\Users\\刘宇\\OneDrive - CUHK-Shenzhen\\桌面\\JAP_GPT\\Test1_new\\test 1 paper\\Test 1 Question Paper.docx"
    sample_mistake_analysis = "C:\\Users\\刘宇\\OneDrive - CUHK-Shenzhen\\桌面\\JAP_GPT\\Test1_new\\1155159595 Test 1_sample_mistakes_analysis.doc"
    Revised_newpaper_folder = "C:\\Users\\刘宇\\OneDrive - CUHK-Shenzhen\\桌面\\JAP_GPT\\Test1_new\\Revised_newpaper_folder"
    Mistake_database = "C:\\Users\\刘宇\\OneDrive - CUHK-Shenzhen\\桌面\\JAP_GPT\\Test1_new\\Mistake_database"
    matched_knowledge_points_folder = "C:\\Users\\刘宇\\OneDrive - CUHK-Shenzhen\\桌面\\JAP_GPT\\matched knowledge points"
    ...
    processor = DocumentProcessor(
        input_folder, 
        output_folder, 
        output_analysis_folder,
        Revised_newpaper_folder, 
        Mistake_database, 
        material_folder, 
        matched_knowledge_points_folder
    )

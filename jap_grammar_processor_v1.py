...
def has_multiple_correct_answers(text, llm, knowledge_point):
    prompt = ChatPromptTemplate.from_template(
        ...
        "An importance notice: once the answer is correct grammatically, please regard it as a correct answer. In other words, never take situational tone error in your consideration. \n"
    ...
    input_data = {'new_paper': text, 'knowledge_point':knowledge_point}
...
def has_stem_errors(text, llm, knowledge_point):
    prompt = ChatPromptTemplate.from_template(
        "You are an experienced Japanese N4/N5 examiner reviewing the following multiple-choice questions:\n\n"
        "the questions are used to test {knowledge_point} \n"
        "{new_paper}\n\n"
...
def stem_options_error(text, llm, knowledge_point):
    prompt = ChatPromptTemplate.from_template(
        "You are an experienced Japanese N4/N5 examiner reviewing the following multiple-choice questions:\n\n"
        "the questions are used to test {knowledge_point} \n"
        "{new_paper}\n\n"
        "Check if any question has **stem-options problem**. This means that after fill in the blank, the stem and the options have some part of overlap causing the overall sentence incorrect.\n"
        "Example: 友達が来たら（　　　　　）パーティーを始めます。 1. 行ったら 2. 来なかったら 3. 来たら 4. 行かなかったら. \n"
        "the '来たら' in stem overlaps with the '来なか' and '来たら' in options leading to redundence meaning. \n"
        "If at least one question has multiple valid correct answers, respond with 'True'. Otherwise, respond with 'False'.\n"
        "Your output must be exactly 'True' or 'False', nothing else."
    )
    chain = LLMChain(llm=llm, prompt=prompt)
...
def question_revise_simple(rows, filename, output_dir, revised_newpaper_folder, knowledge_point, max_iterations=5, model='gpt-4o', temperature=0.6):
    ...
    prompt_revise = ChatPromptTemplate.from_template(
        f'''
Here are the new generated Japanese practice questions related to {knowledge_point}: {rows}
You are an experienced Japanese N4/N5 examiner tasked with reviewing and ensuring that all multiple-choice test questions are in high quality and meet the following criteria:

1. No duplicate questions: Ensure that all questions are unique. If a question is repeated or too similar to another, please revise it to create a new question with a distinct structure or context to test the knowledge point {knowledge_point}. Provide specific suggestions on how to modify repeated questions.
...
3. No duplicate correct answers: Ensure that only one answer is correct for each question. If one option is not semantically natural and logically appropriate but grammatically correct, please regard it as an option. If two options could be correct following the rule above, modify the options to ensure that there is only one grammatically. Provide specific suggestions on how to make the answer clear and unambiguous.

4. Grammatical correctness: The title and stem of each question must be grammatically correct. Review for influent sentence structures and revise them to ensure fluency and correctness. If you detect any grammatical errors, please explain how to fix them.
...
'''
    ...
    input_data = {'new_paper': rows, 'knowledge_point': knowledge_point}
    ...
    for iteration in range(max_iterations):
        ...
        for stem_error in range(max_iterations):
            try:
                if has_stem_errors(revised_result, llm, knowledge_point):
                    print(f"iteration - {iteration} stem check iteration {stem_error}, find stem error", stem_error)
                    revised_result = api_call_with_retry(chain, input_data)
                    input_data['new_paper'] = revised_result
                else:
                    print(f"iteration - {iteration} stem check iteration {stem_error}, no stem error", stem_error)
                    break
            except Exception as e:
                print(f"Error in check_for_error: {e}")
        ...
        for duplicate_options in range(max_iterations):
            try:
                if has_duplicate_options(revised_result):
                    print(f"iteration - {iteration} stem check iteration {duplicate_options}, find duplicate options", duplicate_options)
                    revised_result = api_call_with_retry(chain, input_data)
                    input_data['new_paper'] = revised_result
                else:
                    print(f"iteration - {iteration} stem check iteration {duplicate_options}, no duplicate options", duplicate_options)
                    break
            except Exception as e:
                print(f"Error in check_for_error: {e}")
        ...
        for stem_option_error in range(max_iterations):
            try:
                if stem_options_error(revised_result, llm, knowledge_point):
                    print(f"iteration - {iteration} stem check iteration {stem_option_error}, stem_option_errors", stem_option_error)
                    revised_result = api_call_with_retry(chain, input_data)
                    input_data['new_paper'] = revised_result
                else:
                    print(f"iteration - {iteration} stem check iteration {stem_option_error}, no stem_option_error", stem_option_error)
                    break
            except Exception as e:
                print(f"Error in check_for_error: {e}")
        ...
        for multiple_correct_answers in range(max_iterations):
            try:
                if has_multiple_correct_answers(revised_result, llm, knowledge_point):
                    print(f"iteration - {iteration} stem check iteration {multiple_correct_answers}, find multiple_correct_answers", multiple_correct_answers)
                    revised_result = api_call_with_retry(chain, input_data)
                    input_data['new_paper'] = revised_result
                else:
                    print(f"iteration - {iteration} stem check iteration {multiple_correct_answers}, no multiple_correct_answers", multiple_correct_answers)
                    break
            except Exception as e:
                print(f"Error in check_for_error: {e}")
        ...
        for duplicate_questions in range(max_iterations):
            try:
                if has_duplicate_questions(revised_result):
                    print(f"iteration - {iteration} stem check iteration {duplicate_questions}, find duplicate_questions", duplicate_questions)
                    revised_result = api_call_with_retry(chain, input_data)
                    input_data['new_paper'] = revised_result
                else:
                    print(f"iteration - {iteration} stem check iteration {duplicate_questions}, no duplicate_questions", duplicate_questions)
                    break
            except Exception as e:
                print(f"Error in check_for_error: {e}")
